...
export default function UploadArea() {
  ...
  useEffect(() => {
    ...
    const fetchAndParse = async () => {
      ...
      const candidates = [withBasePath("/BiomeTags.md")];
  ...
  useEffect(() => {
    const loadProcessedFiles = async () => {
      try {
        const { default: getStorage } =
          await import("@/utils/indexedDBStorage");
  ...
  const TABLE_COLUMNS = [
    ...
    {
      key: "spawnablePositionType",
      label: "Spawnable Position Type",
      sortable: true,
    },
  ...
  const allSpawnData = fileReports
    ...
    .flatMap((r) =>
      r.data.map((d) => ({
        ...d,
        // Backward compatibility: normalize old 'context' field to 'spawnablePositionType'
        spawnablePositionType: d.spawnablePositionType || d.context || "none",
        sourceFile: r.name,
      })),
    );
  ...
  const SEARCH_FIELDS = [
    ...
    { value: "spawnablePositionType", label: "Spawnable Position Type" },
    ...
    >
          ...
          <div className="overflow-hidden rounded-lg border border-gray-700/50 bg-[#2a2a2a]">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-linear-to-r from-gray-800/50 to-gray-700/50">
      ...
      {hoverTooltip.visible && (
        <div
          ...
          className="z-1000 max-w-md text-[12px] text-white bg-[#0b0b0c]/95 backdrop-blur-sm border border-gray-700/60 rounded-lg shadow-2xl"
        >
